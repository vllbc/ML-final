...
class HTFN(nn.Module):
    def __init__(
        ...
        self.short_stream = nn.Sequential(
            nn.Conv1d(
                input_dim, gru_hidden_dim, kernel_size=short_kernel_size, padding="same"
            ),
            nn.ReLU(),
            nn.Dropout(dropout),
        )
        ...
        self.attention_short = nn.MultiheadAttention(
            gru_hidden_dim, cross_attention_heads, dropout=dropout, batch_first=True
        )
        self.attention_mid = nn.MultiheadAttention(
            gru_hidden_dim, cross_attention_heads, dropout=dropout, batch_first=True
        )
        self.attention_long = nn.MultiheadAttention(
            gru_hidden_dim, cross_attention_heads, dropout=dropout, batch_first=True
        )
    ...
    def forward(self, src):
        ...
        short_out = self.short_stream(src_permuted).permute(
            0, 2, 1
        )  # -> (batch, seq, gru_hidden)
