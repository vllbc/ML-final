...
def plot_predictions(model_name, horizon):
    ...
    assert os.path.exists(preds_path) and os.path.exists(labels_path), (
        f"Prediction or label file not found for {model_name}, horizon {horizon}."
    )
    ...
    plt.plot(
        predictions[0, :], label="Prediction", color="red", linestyle="--", marker="."
    )
