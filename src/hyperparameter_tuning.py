...
def objective(
    trial, model_name, train_loader, test_loader, power_scaler, input_dim, horizon
):
    ...
    if model_name == "LSTM":
        ...
        transformer_dim_feedforward = trial.suggest_categorical(
            "transformer_dim_feedforward", [256, 512, 1024]
        )
    ...
    for epoch in range(EPOCHS):
        ...
        if (epoch + 1) % EVAL_INTERVAL == 0:
            mse, mae, preds, labels = evaluate_model(
                model, test_loader, power_scaler, return_preds=True
            )
...
def main():
    ...
    for window in windows_to_tune:
        ...
        if not test_loader:
            print(
                f"Test loader is empty for window size {window}. Cannot run hyperparameter tuning. Skipping."
            )
            continue
