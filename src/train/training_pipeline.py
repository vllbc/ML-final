...
def create_sliding_windows(data, input_window_size, output_window_size):
    ...
    for i in range(len(data) - input_window_size - output_window_size + 1):
        ...
        output_slice = data[
            i + input_window_size : i + input_window_size + output_window_size, 0
        ]
...
def get_data_loaders(input_window, output_window, train_path, test_path, batch_size):
    ...
    X_train, y_train = create_sliding_windows(
        scaled_train_data, input_window, output_window
    )
    X_test, y_test = create_sliding_windows(
        scaled_test_data, input_window, output_window
    )
...
def train_model(
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % eval_interval == 0:
            ...
            print(
                f"Epoch [{epoch + 1}/{num_epochs}], Train Loss: {avg_loss:.4f}, LR: {current_lr:.6f}"
            )
            if test_dataloader:
...
def save_results(
    model_name, horizon, trained_model, train_loader, test_loader, power_scaler
):
    ...
    _, _, test_preds_inv, test_labels_inv = evaluate_model(
        trained_model, test_loader, power_scaler, return_preds=True
    )
    ...
    _, _, train_preds_inv, train_labels_inv = evaluate_model(
        trained_model, train_loader, power_scaler, return_preds=True
    )
    np.save(f"results/{model_name}_train_preds_horizon_{horizon}.npy", train_preds_inv)
    np.save(
        f"results/{model_name}_train_labels_horizon_{horizon}.npy", train_labels_inv
    )
